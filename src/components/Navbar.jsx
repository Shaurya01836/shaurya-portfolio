...
const Navbar = ({ toggleTheme, isDarkMode }) => {
  return (
    <nav className="fixed top-6 right-6 md:top-5 md:right-60 z-50 flex gap-4">
      <Link
        to="/"
        className="flex items-center gap-2 text-gray-600 dark:text-gray-300 hover:text-black dark:hover:text-white font-semibold text-lg transition-colors bg-white/50 dark:bg-black/50 backdrop-blur-sm px-4 py-2 rounded-md border border-gray-200 dark:border-[#1F1F1F]"
      >
        <span>Portfolio</span>
      </Link>
      <Link
